...
void main() {
  ...
  test('Wrong password (indirectly wrong key) throws opaque error', () async {
    ...
    expect(() => cryptoService.decrypt(encrypted, wrongKey), 
           throwsA(isA<Exception>().having((e) => e.toString(), 'msg', contains('Unable to decrypt data.'))));
  });

  test('Wrong nonce throws opaque error', () async {
    final key = SecretKey(randomService.generateKey());
    final encrypted = await cryptoService.encrypt(Uint8List.fromList([1, 2, 3]), key);
    
    final wrongNonce = randomService.generateNonce();
    final tampered = CryptoResult(encrypted.ciphertext, wrongNonce, encrypted.mac);
    
    expect(() => cryptoService.decrypt(tampered, key), 
           throwsA(isA<Exception>().having((e) => e.toString(), 'msg', contains('Unable to decrypt data.'))));
  });
  ...
  test('Tampered ciphertext throws opaque error', () async {
    ...
    final tampered = CryptoResult(tamperedCiphertext, encrypted.nonce, encrypted.mac);
    expect(() => cryptoService.decrypt(tampered, key), 
           throwsA(isA<Exception>().having((e) => e.toString(), 'msg', contains('Unable to decrypt data.'))));
  });
  ...
  test('Tampered auth tag throws opaque error', () async {
    ...
    final tampered = CryptoResult(encrypted.ciphertext, encrypted.nonce, tamperedMac);
    expect(() => cryptoService.decrypt(tampered, key), 
           throwsA(isA<Exception>().having((e) => e.toString(), 'msg', contains('Unable to decrypt data.'))));
  });
  ...
  test('Corrupted metadata (DEK unwrapping) throws opaque error', () async {
    ...
    final keyManagement = KeyManagementService(storage, cryptoService, randomService);
    ...
    expect(() => keyManagement.unwrapDek(kek), 
           throwsA(isA<Exception>().having((e) => e.toString(), 'msg', contains('Unable to unlock vault.'))));
  });
  ...
  test('10,000 consecutive encryptions (benchmark)', () async {
    ...
    stopwatch.stop();
    expect(stopwatch.elapsedMilliseconds, lessThan(5000)); // Should be extremely fast
  });
