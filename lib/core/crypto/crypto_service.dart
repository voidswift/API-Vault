...
class CryptoService {
  ...
  Future<CryptoResult> encrypt(Uint8List plaintext, SecretKey key) async {
    try {
      ...
      return CryptoResult(Uint8List.fromList(box.cipherText), Uint8List.fromList(nonce), Uint8List.fromList(box.mac.bytes));
    } finally {
  ...
  Future<Uint8List> decrypt(CryptoResult encrypted, SecretKey key) async {
    try {
      final box = SecretBox(encrypted.ciphertext, nonce: encrypted.nonce, mac: Mac(encrypted.mac));
